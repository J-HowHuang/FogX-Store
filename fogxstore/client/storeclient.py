...
class FogXStoreClient:
    ...
    def get_dataset(self, dataset_name) -> pa.Table:
        upload_descriptor = pa.flight.FlightDescriptor.for_path(f"{dataset_name}.lance")
        flight = self.client.get_flight_info(upload_descriptor)
        descriptor = flight.descriptor
        logging.debug("Path:", descriptor.path[0].decode('utf-8'), "Rows:", flight.total_records, "Size:", flight.total_bytes)
